...
import { patternsNavigation, allPatternsPages } from "@/lib/patterns-navigation";
...
export function DocsMobileNav() {
  ...
  const currentPage = useMemo(() => {
    const page = allPatternsPages.find((page) => page.href === pathname);
    return page ?? allPatternsPages[0];
  }, [pathname]);
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="overflow-y-auto p-6">
        <SheetTitle className="mb-6">Patterns</SheetTitle>
        <nav className="space-y-6">
          {patternsNavigation.map((section) => (
