...
`,
  ...
  for (const pattern of selected) {
    ...
    for (const file of pattern.files) {
      ...
      // Fix API route references in page files so they point to the namespaced paths
      let content = file.content;
      if (file.path === "app/page.tsx" && selected.length > 1) {
        // Find all /api/* references and namespace them under the pattern slug
        // Handles both `api: "/api/..."` (useChat transport) and `fetch("/api/..."` patterns
        const apiPaths = pattern.files
          .filter((f) => f.path.startsWith("app/api/"))
          .map((f) => f.path.replace("app/api/", "/api/"));
        for (const apiPath of apiPaths) {
          // Replace /api/chat/route.ts → /api/chat, then remap to /api/slug/chat
          const endpoint = apiPath.replace(/\/route\.ts$/, "");
          const namespacedEndpoint = endpoint.replace("/api/", `/api/${slug}/`);
          content = content.replaceAll(endpoint, namespacedEndpoint);
        }
      }
...
function generatePackageJson(selected: PatternMeta[]): string {
  ...
  // Add pattern-specific deps by scanning file content for known imports
  const allContent = selected.flatMap((p) => p.files.map((f) => f.content)).join("\n");
...
This application was generated by [AI SDK Patterns](${process.env.NEXT_PUBLIC_SITE_URL || "https://ai-sdk-patterns.vercel.app"}) by composing ${selected.length} patterns.
...
- [AI SDK Patterns](${process.env.NEXT_PUBLIC_SITE_URL || "https://ai-sdk-patterns.vercel.app"})
`;
}
